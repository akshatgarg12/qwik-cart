...
export const Product = component$(({item} : ProductProps) => {
  ...
  const ifProductAlreadyInCart = cart.value.filter(({item}) => item.id === id).length
  const initialProductCount = ifProductAlreadyInCart ? cart.value.filter(({item}) => item.id === id)[0].count : 0
  ...
  const decreaseItemCountInCart = $(async (item:Item) => {
    if(ifProductAlreadyInCart){
      ...
      if(removeProduct){
        cart.value = updatedCart.filter((cartObj) => cartObj.item.id !== item.id)
      }else{
        cart.value = [...updatedCart]
      }
    }
